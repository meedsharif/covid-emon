...
function populateLocations() {
	Object.entries(countriesWithCountryCodes).forEach(([country_code, country]) =>
		populateLocation(country, country_code)
	);
}

mapboxgl.accessToken =
	'sk.eyJ1IjoiZWJpc3dhcyIsImEiOiJja2kzYTJzZ2c3OWFlMnpsNjhwOGtudGY1In0.3G4azsQe7aPPeb9zqZzzXw';
...
function renderMap() {
	mapboxgl.accessToken =
		'pk.eyJ1IjoiZWJpc3dhcyIsImEiOiJja2kzOXhzM3cxaHV5MnlwZWxqMjJuczBsIn0.m-kXuAHDri3TfKo2sXiewg';
		...
		map.addSource('places', {
			...
			data: {
				...
				features: await Promise.all(
					coronaData.locations.map(async (location) => {
...
function renderDetailsForSelectedLocation(event) {
	...
	const locationCoronaDetails = coronaData.locations.reduce(
		function (accumulator, currentLocation) {
